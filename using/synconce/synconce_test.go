...
// TestUsingSyncOnce показывает пример как используется sync.Once для однократного выполнения.
// sync.Once — это структура из стандартной библиотеки Go (sync), которая гарантирует, что одна и та же операция
// выполнится только один раз, даже если несколько goroutine пытаются вызвать её одновременно.
