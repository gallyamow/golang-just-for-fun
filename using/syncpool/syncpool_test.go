...
// TestUsingSyncPool показывает пример использования sync.Pool.
//
// Для чего он:
// Эффективного повторного использования временных объектов.
//
// Как с ним работать:
// 1) pool.Get().(*SomeType) - когда нужно воспользоваться
// 2) pool.Put(some) - когда воспользовались, перед этим можно сделать какой-то reset.
//
// Как реализован:
